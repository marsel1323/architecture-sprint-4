...
- **Отсутствие метрик и мониторинга**:
  - Причины: недостаток инструментов для сбора и анализа данных. 
  - Последствия: неизвестно, в каких конкретных местах системе плохо, трудности в выявлении узких мест. 
  - Решение: добавить следующие метрики:
    - RPS (Requests per Second): показывает количество запросов на API. 
    - Latency (Время ответа API): выявляет задержки в работе сервисов. 
    - Количество сообщений в очереди (RabbitMQ): помогает мониторить состояние очередей. 
    - Загрузка CPU и памяти: критически важна для мониторинга ресурсов сервисов MES и CRM.
---
...
1.	Оптимизация MES API для управления внешней нагрузкой:
   - Причина: устранение проблем с производительностью и улучшение стабильности работы MES.
   - Влияние: снижение времени отклика API на 50%, уменьшение нагрузки на базу данных и улучшение работы системы.

2. Мониторинг и трейсинг (RabbitMQ + Jaeger):
   ...
   - Влияние: увеличение скорости отклика API и уменьшение времени загрузки MES.

Проблема с MES API является наиболее критичной и её решение даст максимальный эффект на производительность системы
